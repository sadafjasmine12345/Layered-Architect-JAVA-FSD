...
public class BookDAO {
	...
	private static String bookQry="select * from book";//write query in uppercase also because database is case-sensitive
	/*********Get books data**********/
	public ArrayList<Book>getBooks() throws ClassNotFoundException, SQLException{
		...
		//3.return booklist
		//data from rset we are pushing into booklist
		try {
			//open the connection
			...
			while(resultSet.next()) {
				...
				//set the pojo with retrieved values from the row -plain old java object
		...
		public void showBookList(ArrayList<Book>bookList2) {
			...
			/**************updating book************/
			public boolean updateBook(int id) throws SQLException,ClassNotFoundException{//id is argument
				pstatement=null;
				connection=DbConnector.createConnection();
				String updPriceQry="update book set price=price+price*0.10 where id=?";
				pstatement=null;
				pstatement=connection.prepareStatement(updPriceQry);
				pstatement.setInt(1,id);//id is a argument of type boolean
				int rows=pstatement.executeUpdate();
